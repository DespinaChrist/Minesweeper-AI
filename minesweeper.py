...
class Minesweeper():
    ...
    def __init__(self, height=8, width=8, mines=8):
        ...
        self.board = []
        for i in range(self.height):    # 0 to height - 1
            row = []
            for j in range(self.width): # 0 to width - 1
                row.append(False)
...
class MinesweeperAI():
    ...
    def mark_cells(self):
        ...
        while (True):
            ...
            knowledge_added = self.infer_new_knowledge()

            if not (knowledge_added or knowledge_changed):
                break
    ...
    def infer_new_knowledge(self):
        ...
        (using the subset method described in the Background), then those sentences are
        added to the knowledge base as well.    
        '''
        knowledge_copy = self.knowledge
        knowledge_added = False
        for pair in combinations(knowledge_copy, 2):
            sentence1 = Sentence(pair[0].cells, pair[0].count)
            sentence2 = Sentence(pair[1].cells, pair[1].count)
    ...
    def make_random_move(self):
        ...
        unknown_cells = set()
        for i in range(self.height):    
            unknown_cells.update(set((i,j) for j in range(self.width)))
        unknown_cells -= self.mines
        unknown_cells -= self.moves_made
        if unknown_cells != set():
